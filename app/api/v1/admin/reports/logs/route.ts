...
// ============================================================================
// PROXY: app/api/v1/admin/reports/logs/route.ts -> Django backend
// ============================================================================

const DJANGO_API_BASE_URL = process.env.BACKEND_URL ?? 'http://localhost:8000';
const DJANGO_REPORT_LOGS_ENDPOINT = `${DJANGO_API_BASE_URL}/api/v1/admin/reports/logs/`;
...
export async function GET(request: NextRequest) {
  ...
  try {
    djangoResponse = await fetch(`${DJANGO_REPORT_LOGS_ENDPOINT}?${djangoParams.toString()}`, {
      method: 'GET',
      headers: {
  ...
  if (!djangoResponse.ok) {
    ...
    console.error(
      `Django returned ${djangoResponse.status} for ${DJANGO_REPORT_LOGS_ENDPOINT}:`,
      rawBody
    );
    let body: unknown;
    try {
